...
ADMIN_UID = "1200909333"
GUEST_UID = "14478459328"
GUEST_PASSWORD = "EF76FC9D7770135CA365CCF2A592342D6BCED3BFCDD8A5930BD68224790FD26A"
region = 'IN'
...
[FFFFFF]CHANDUsmove , furiousslam , moonflip , wigglewalk
...
async def reject_spam_loop(target_uid, key, iv):
    ...
    while reject_spam_running and count < max_spam:
        try:
            packet1 = await CHANDU1(target_uid, key, iv)
            packet2 = await CHANDU(target_uid, key, iv)
...
async def CHANDU(client_id, key, iv):
    banner_text = f""" 
.
.
.
...
async def CHANDU1(client_id, key, iv):
    gay_text = f""" 
.
.
.
...
async def convert_kyro_to_your_system(target_uid, chat_id, key, iv, nickname="CHANDU", title_id=904990072):
    try:
        fields = {
            1: 1,
            2: {
...
async def send_kyro_title_adapted(chat_id, key, iv, target_uid, nickname="CHANDU", title_id=905190079):
    try:
...
async def send_all_titles_once(chat_id, key, iv, target_uid, whisper_writer, nickname="CHANDU"):
    titles = [
...
async def handle_title_final(inPuTMsG, uid, chat_id, key, iv, region, chat_type=0):
    ...
    try:
        if not whisper_writer:
            return
        title_packet = await convert_kyro_to_your_system(target_uid, chat_id, key, iv, "CHANDU", selected_title)
        if title_packet and whisper_writer:
            whisper_writer.write(title_packet)
            await whisper_writer.drain()
    except Exception:
...
async def send_sticker(target_uid, chat_id, key, iv, nickname="CHANDUxBOT"):
    try:
            ...
            while True:
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        insquad = True
                        try:
                            welcome_message = """[B][C][FFFFFF]Welcome to CHANDU Bot
...
[B][C][FFFFFF]Telegram : @chandu_zx
[B][C][FFFFFF]Contact : @chandu_zx 
...
[B][C][FFFFFF]Status: Online & Active"""
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        ...
                        message = """[B][C][FFFFFF]Welcome to CHANDU Bot
...
[FFFFFF]• rampage • cannibal • CHANDU
...
[FFFFFF]
1[C]2[C]0[C]0[C]9[C]0[C]9[C]3[C]3[C]3[C] & 1[C]0[C]7[C]2[C]1[C]6[C]7[C]2[C]8[C]9[C]

[C][B][00FF00]💼 BUSINESS:
[C][B][FFFFFF] this bot?
[C][B][FFFFFF]DM me for This!
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            if len(parts) < 3:
                                ...
                                await dl_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                try:
                                    success = await Room_Spam(target_uid, room_id, "CHANDU", key, iv)
                                    if success:
                        ...
                        if inPuTMsG.strip().startswith('/sticker'):
                            packet = await send_sticker(uid, chat_id, key, iv, nickname="CHANDU")
                            if packet:
...
"""
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                ...
                                await dl_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        if inPuTMsG.strip().lower() == "chandu bhai mera uid do":
...
async def MaiiiinE():
    ...
    Uid, Pw = '4418614960', '3701C6CB7C47804D4C3A7422C7D70FE567A10B21594B478C35D4A62019ECFB18'
    ...
    os.system('clear')
    print(render('CHANDU', colors=['white', 'green'], align='center'))
    print('')
    print("🤖 CHANDU BOT - ONLINE")
    print("=" * 50)
    print(f"🔹 UID: {TarGeT}")
